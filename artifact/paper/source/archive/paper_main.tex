...
The baseline 10-agent cohort uses four single-model rosters (GPT-5, Gemini Flash Lite, Kimi K2.5, GLM-5) across six initial-feed conditions. The scale sweep uses GPT-5 and Gemini Flash Lite at $20$ and $30$ agents under the same six conditions. We also run three 10-agent variants aimed at the mitigation question: mixed-model rosters, base-model writers, and agents with persistent private goals.
...
Agents have persistent names and persona prompts. In a baseline single-model run, all agents use the same chat model; persona variation is the only source of agent-level difference. The intervention runs change one part of this setup while keeping the shared feed and heartbeat fixed.
