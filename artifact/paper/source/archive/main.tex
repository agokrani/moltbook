...
\begin{abstract}
LLM agents increasingly operate in shared environments where one agent's output becomes another agent's context. Because they share pretraining corpora and alignment recipes, this homogenization can compound as their text re-enters training data and public discourse. Diversity loss of this kind has already been reported in debate, conformity, and aligned-population settings, but prior work has not tested whether it can be engineered away in shared-feed deployments. We study this in Moltbook, a Reddit-like environment for autonomous agents, testing three interventions: mixing models, using base models for content generation, and assigning persistent private goals. Lexical, compression, embedding, and LLM-judge metrics agree on the baseline pattern: the feed becomes larger but less diverse, and each run develops its own local attractors: reusable phrases, templates, roles, and frames that propagate across agents. Across 24 first-hour runs spanning four model families, none of the three interventions reliably preserves feed diversity: mixed models still show rising LLM collapse, base-model writers reduce some exact phrase repetition but not feed-level convergence, and persistent goals shift repetition from shared catchphrases toward each agent's own templates. Entropy collapse therefore appears to be a structural property of shared-feed agent societies, and mitigating it likely requires loop-level rather than per-agent interventions.
\end{abstract}
\begin{figure*}[t]
  \centering
...
LLM agents are increasingly deployed in shared environments where one agent's output becomes another agent's context: multi-agent debate, collaborative coding, evaluation pipelines, role-playing simulations, and now agent-native social platforms \citep{park2023generative,li2023camel,wu2023autogen,hong2023metagpt}. The scale of these deployments is growing, and so is the share of public text that originates from LLM agents rather than humans.

The agents populating these environments are trained on overlapping pretraining corpora, aligned with similar instruction-tuning and RLHF recipes, and decoded with similar sampling defaults. As they read and write back into a shared feed, they may converge on a narrow band of lexical patterns, framings, and topics that reflect their shared training rather than the diversity of the inputs they are nominally responding to. When agent-generated text then re-enters the public web, through scraping, downstream training data, or direct consumption by future models and users, that homogenization compounds across generations \citep{shumailov2024modelcollapse}. Diversity in agent-produced public text is therefore not only an aesthetic property of the feed itself, but a property of the next round of training corpora and of the discourse readers and models will inherit downstream.

Multi-agent diversity loss is itself not new. Studies of LLM debate, conformity, conventions, and aligned-population diversity already report convergence on phrases, opinions, and reasoning paths \citep{ashery2025conventions,cho2025herd,choi2025conformity,chen2026diversitycollapse,patel2026representational}. We refer to this failure mode as \emph{\collapse{}}: a narrowing of discourse in which later posts become more redundant, more compressible, and more concentrated in a few lexical, semantic, or rhetorical regions. Our question is the natural follow-up: \emph{can it be engineered away?} If entropy collapse in shared-feed agent societies is a tractable target, then plausible interventions such as changing the model population, swapping in different writers, or giving agents persistent commitments outside the feed should restore diversity.

We test this in Moltbook, a Reddit-like platform for autonomous LLM agents, across 24 first-hour 10-agent runs spanning four models (GPT-5, Gemini Flash Lite, Kimi K2.5, GLM-5) and six initial-feed conditions. Three independent measurements of the accumulated feed agree that diversity declines in the baseline: cumulative Distinct-5 declines in 24/24 runs, gzip ratio declines in 24/24, and an LLM-judge collapse score rises in 23/24. We then evaluate three natural mitigations: mixed-model rosters, base-model content generators, and persistent private goals. Each changes the \emph{form} of collapse rather than removing it: mixed-model rosters still synchronize on shared frames, base-model writers reduce some exact phrase repetition but not feed-level convergence, and private goals shift repetition from shared catchphrases toward individual agents' own templates. Entropy collapse in shared-feed agent societies therefore appears to be a property of the interaction loop itself, not a side effect of any one model or design choice.

\section{Related Work}
\label{sec:related}

\paragraph{Agent social systems.}
Most multi-agent LLM research studies coordination rather than open-ended communication, placing agents in simulated worlds, role-playing settings, software teams, debate protocols, or evaluation pipelines \citep{park2023generative,li2023camel,chen2023agentverse,wu2023autogen,hong2023metagpt,qian2024chatdev,zhou2024sotopia,du2023debate,chen2024reconcile,chan2024chateval}. These settings usually measure task success, coordination, reasoning, or believability. Moltbook raises a different question: what happens to the public text stream when agents read and write in the same space? Studies of the public Moltbook platform report recurring topics and narratives, concentrated attention, low reciprocity, subcommunities, and norm-like behavior \citep{jiang2026moltbook,demarzo2026moltbook,li2026moltbook,price2026moltbook,houji2026moltbook,lin2026siliconsociology,manik2026openclaw,feng2026moltnet,zerhoudi2026form}. Because the public platform is open, those traces can include platform incentives and human steering. Our controlled deployment removes that ambiguity.

\paragraph{Convergence and text diversity.}
The closest background comes from social influence and text-diversity measurement. Public signals and group pressure can shift expressed judgments \citep{asch1951,bikhchandani1992,muchnik2013social}; LLM-agent studies report conventions, herding, conformity, and losses of semantic or reasoning diversity \citep{ashery2025conventions,cho2025herd,choi2025conformity,chen2026diversitycollapse,patel2026representational}. Text-generation work measures degeneration and repetition with distinct-$n$, Self-BLEU, MAUVE, compression, and embedding-space diversity \citep{li2016diversity,zhu2018selfbleu,holtzman2020curious,pillutla2021mauve,tevet2021evaluating,yun2025format}. We borrow these tools, but apply them to a running social feed. This is not training-time model collapse \citep{shumailov2024modelcollapse}: no weights are updated. The feed narrows because agent outputs become the next round of inputs.
...
We run the experiments in a controlled Moltbook-style platform. For this paper, the important object is not the database or web client but the feedback loop: agents read a shared Reddit-like feed, then write back to it. Each run fixes the model roster, personas, heartbeat, rate limits, and initial feed. Humans can watch the feed, but they do not post, vote, comment, or moderate during any run.
...
\begin{equation}
    ...
    \label{eq:loop}
\end{equation}
The action set is fixed across the paper:
\[
...
A \emph{run} is one independent deployment with a fixed roster, model setting, and initial feed. All main runs use a one-minute heartbeat and are analyzed over the first hour. Seed posts initialize the feed but are excluded from all metrics.

The core of our design is three interventions targeting entropy collapse: mixed-model rosters, base-model content generators, and persistent private goals. Each is run on 10 agents with a one-minute heartbeat across six initial-feed conditions. As reference conditions for the baseline phenomenon, we additionally run four single-model 10-agent cohorts (GPT-5, Gemini Flash Lite, Kimi K2.5, GLM-5) across the same six conditions, plus a scale sweep with GPT-5 and Gemini Flash Lite at $20$ and $30$ agents.

\subsection{Initial feed conditions}
\label{subsec:conditions}

The initial feed conditions in Table~\ref{tab:feed-conditions} separate spontaneous narrowing from seed-driven narrowing. In the empty condition, agents begin with no seeded topic. In the seeded conditions, the experiment inserts pre-written posts before the agents start. Seeds are visible to agents but removed from every reported metric.
...
Agents have persistent names and persona prompts. We study three interventions, each changing one part of the agent setup while keeping the shared feed and heartbeat fixed:

\begin{itemize}\itemsep1pt
    \item \textbf{Mixed-model rosters} put different chat models in the same 10-agent feed.
    \item \textbf{Base-model writers} keep an instruction-tuned controller for action selection, but use a base model to draft post or comment text. Drafts rewritten by the controller are rejected.
    \item \textbf{Private-goal agents} receive a stable outside track, such as coding, forecasting, fitness, hadith commentary, or cinema, and are instructed to check that track before browsing the feed.
\end{itemize}

As a reference condition, baseline single-model runs use the same chat model for all 10 agents, so that persona variation is the only source of agent-level difference.

\subsection{Collapse target and metrics}
\label{subsec:metrics}

Let $P_t$ be the multiset of agent-generated public posts in time window $t$, and let $P_{\le t} = \bigcup_{s \le t} P_s$ be the accumulated feed up to that window. For a diversity measure $\mathcal{D}$, we say a run collapses on $\mathcal{D}$ if $\mathcal{D}(P_{\le t})$ falls over time. For the LLM-judge index below, the direction is reversed: higher values mean stronger collapse.

We use three cumulative metrics. \textbf{Distinct-5} is the fraction of unique 5-grams in $P_{\le t}$; lower values mean new posts add fewer new local word sequences. \textbf{Gzip ratio} is compressed size divided by raw size for the concatenated agent-authored text; lower values mean the feed is easier to compress. \textbf{LLM-judge collapse index} scores each post for semantic repetition $R$, frame convergence $F$, consensus conformity $G$, template rigidity $T$, and novelty $N$, each on a $1$--$5$ scale. The judge sees anonymized local context but not model, run, condition, agent name, or file path. We combine the fields as
\begin{equation}
\mathrm{CI} = \frac{R + F + G + T + (6 - N)}{5},
\label{eq:ci}
\end{equation}
so higher values mean more repetition, more frame alignment, more conformity, more template rigidity, and less novelty. We report the cumulative mean of $\mathrm{CI}$ up to each cutoff.

We also use two diagnostics that are easier to inspect qualitatively: exact repeated 5-grams, which show concrete phrase attractors, and embedding-space maps, which show whether posts also concentrate semantically.
...
\begin{figure}[t]
  \centering
  \includegraphics[width=\columnwidth]{plots/final/figure_seed_condition_examples.png}
  \caption{Example seed conditions and recurring agent-generated 5-grams.}
  \label{fig:seed-condition-examples}
\end{figure}
A second key point is locality. The repeated phrases do not form one global slogan shared across all runs. They are specific to particular runs and can be shaped by the initial feed condition. Figure~\ref{fig:seed-condition-examples} illustrates this across the three seed-topic conditions: conspiracy seeds make claim-checking formats salient (``Claim (1 line):'' adopted by 8/10 agents over 84 posts), AGI seeds surface skeptical meta-commentary (``none of this is profound,'' 9/10 agents, 27 posts), and tech seeds produce product-pitch templates (``use with the lights off,'' 9/10 agents, 203 posts). In each case the repeated 5-gram is not copied from the seed itself; it is an agent-generated phrase that emerges after initialization. The seed posts shape the discourse context, but the measured attractors are produced and stabilized by the agents.

\subsection{Do Independent Metrics Agree on Feed-Level Collapse?}
\label{subsec:independent-metrics}

Having established concrete examples of collapse inside individual posts, we turn to the feed-level pattern. If these local attractors reflect a broader collapse rather than isolated phrase reuse, independent metrics should show the accumulated feed becoming less diverse or more repetitive over time. Figure~\ref{fig:cumulative-collapse} plots cumulative Distinct-5, gzip ratio, and LLM collapse for 10-agent runs at 15-minute cutoffs, with each metric recomputed over all agent-generated posts observed so far. Lower Distinct-5 and gzip indicate less lexical variety and higher redundancy; higher LLM collapse indicates stronger judged repetition and frame convergence.
...
The run-level pattern is consistent. Across the 24 baseline 10-agent runs, Distinct-5 declines in 24 of 24 runs, gzip ratio declines in 24 of 24 runs, and the LLM collapse index rises in 23 of 24 runs. The accumulated feed becomes larger but less diverse. As interaction proceeds, new 5-grams appear more slowly, repeated structures make the text more compressible, and LLM-judge scores register stronger repetition and frame convergence.

The metrics use different evidence: aggregate lexical variety, model-free compression, and LLM-judge ratings without run metadata. Their agreement makes it unlikely the trend is an artifact of any single metric. Together with the phrase ledger in Figure~\ref{fig:phrase-ledger}, this shows that local attractors leave two signatures: repeated phrases within runs and a collapse in overall feed diversity over time.
...
The same pattern appears in the LLM collapse index. As Table~\ref{tab:private-goals} shows, runs with private goals still increase over time: all 6 GPT-5 runs with private goals have positive deltas. At the same time, the increase is smaller than the GPT-5 baseline in five of six seed conditions. Private goals therefore reduce the feed-wide increase in collapse, but they do not keep the overall discourse open-ended.
...
% Mitigation summary table: commented out (too tall, floats to its own page).
% \begin{table*}[!htbp]
% \centering
% \footnotesize
% \setlength{\tabcolsep}{4pt}
% \renewcommand{\arraystretch}{1.1}
% \begin{tabular}{p{0.13\textwidth}p{0.22\textwidth}p{0.20\textwidth}p{0.22\textwidth}p{0.16\textwidth}}
% \toprule
% Mitigation & Intended mechanism & What improves & What still collapses & Main evidence \\
% \midrule
% Mixed models
% & Different models should yield feed diversity.
% & Slightly weaker phrase adoption in some conditions.
% & LLM collapse still rises; agents synchronize on shared frames.
% & Collapse on all three core metrics. \\
% Base-model generators
% & Base model writes posts/comments; instruction-tuned controller runs the loop.
% & Qwen Base weakens exact phrase repetition; Distinct-5 stays nearly flat.
% & OLMo Base still collapses; gzip and LLM collapse still rise.
% & Qwen helps one signal; OLMo does not. \\
% Private goals
% & Durable source of attention outside the shared feed.
% & Shared catchphrases spread less across agents.
% & Agents settle into their own templates; LLM collapse still rises.
% & Top phrase spreads less; top-agent share rises. \\
% \bottomrule
% \end{tabular}
% \caption{Summary of mitigation results. Each intervention changes the form of collapse, but none reliably preserves feed diversity.}
...
\section{Embedding-cluster maps}
\label{app:embedding-maps}

This appendix contains the embedding-cluster maps for all models. Each panel projects agent-generated posts with MDS after embedding-based clustering; rows show scale, columns show the initial-feed condition, and colors mark per-model clusters (not comparable across models).
