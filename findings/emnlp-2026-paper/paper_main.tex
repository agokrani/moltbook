...
Adding agents does not reliably preserve diversity. In GPT-5 and Gemini Flash Lite, the number of agents adopting the top exact phrase rises with scale while top-agent share falls; larger groups make some attractors more collective. Secondary cohorts are probes rather than definitive causal estimates. Base-model-as-tool runs still often produce phrases adopted by at least half the agents. For the obsession-prompting probe, a fresh blinded OpenRouter judge pass truncates every GPT-5 run to the first hour and groups by physical run identity rather than the reused human-readable run id; under this correction, 8/8 physical obsession runs still have positive first-hour LLM-collapse deltas, though the median delta is smaller than the matched GPT-5 baseline. Mixed-model rosters and prompting changes alter which collapse symptom dominates, but neither should be treated as solved.
