"""
Deploy OLMo-3.1-32B-Think (full RL) on Modal with SGLang.
...
Usage:
    ...
    modal run modal/serve_olmo3_think.py::download_model

    # Deploy (stays running until stopped)
    modal deploy modal/serve_olmo3_think.py

    # Test locally first
    modal run modal/serve_olmo3_think.py

    # Get the endpoint URL after deploy:
    #   https://<your-workspace>--olmo3-think-v1-completions.modal.run
"""
...
model_volume = modal.Volume.from_name("olmo3-think-weights", create_if_missing=True)
MODEL_DIR = "/models"
...
class Model:
    ...
    async def v1_completions(self, request: dict):
        ...
        if messages:
            # Chat mode — apply the model's chat template
