#!/usr/bin/env python3
"""Analysis script for threshold dose-response experiment (factual 0→5).

Loads exports from exports/th-f{0..5}-run{01..NN}/ and topic-mapping.json,
...
Figures saved to experiments/conspiracy/figures/.

Usage:
    python3 experiments/conspiracy/analyze_threshold.py [--export-dir exports]
"""
...
def main():
    parser = argparse.ArgumentParser(description="Analyze threshold dose-response experiment")
    ...
    print("=" * 60)
    print("  THRESHOLD DOSE-RESPONSE ANALYSIS (Factual 0→5)")
    ...
    ax.set_title(f"Dose-Response: Factual Post Count → Engagement\n({n_reps} replications, {len(world)} world posts)", fontsize=13)
    ...
    if len(gaps) >= 2:
        ...
        ax.set_title(f"Score Gap vs Factual Dose ({n_reps} replications)", fontsize=14)
