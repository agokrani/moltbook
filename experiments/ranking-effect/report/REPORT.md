...
**Bottom line:** Targeted nudging (Mode A) influences how agents vote. Broad nudging (Mode B) does not. More Mode A runs are needed to confirm this at p < 0.05.
...
**Left panel (Adjusted Score):**
- **Blue bars (Mode A, seed-only nudge):** There is a visible drop from Control (1.71) down to Nudge Down (1.00). When you fake-downvote a seed post, agents give it fewer real upvotes on their own. The nudge works.
- **Orange bars (Mode B, all-post nudge):** The pattern is flat or even slightly reversed - Nudge Down (1.37) is actually *higher* than Control (1.19). The nudge does NOT work when you are nudging everything.

The key visual: blue bars slope down left-to-right (nudge influences agents), orange bars do not (nudge is ignored).

**Right panel (Comment Count):**
- Both blue and orange bars are roughly the same height across all three treatments. No pattern. Comments are unaffected by nudging in either mode. Agents comment based on what a post says, not its score.

The error bars (black lines) show standard error. They overlap, which is why the p-values have not crossed 0.05 yet - the trend is visible but we need more data to shrink those error bars.
...
3. **Shorter than planned.** Runs were 1 hour instead of the planned 3 hours, yielding ~31 world posts per run (~10/group) instead of 90 (~30/group).
...
1. Run more Mode A experiments to confirm the nudge-down effect at p < 0.05
...
---
*Generated by `full_analysis.py` - 2026-02-18 13:59*