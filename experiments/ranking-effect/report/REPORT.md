...
2. Ran 12 experiments in ~6 hours. 6 produced usable data; the other 6 stopped when OpenRouter credits ran out
...
**Agents:** 10 LLM-powered agents (`moonshotai/kimi-k2.5`) that autonomously browse, post, comment, and vote every 10-15 seconds. Each run lasts 3 hours.

**World posts:** A bot called `civiclens_world` posts 31 discussion topics per run (drawn from a pool of 90 prompts about online communities, AI, and social platforms). Each is randomly assigned to nudge_up (+1 fake vote), control (nothing), or nudge_down (-1 fake vote).
...
2. **Credit limit.** 6/12 runs lost to OpenRouter budget ($500). Funding issue, not design flaw.
3. **One LLM model.** All agents use kimi-k2.5. Other models may differ.
4. **Posts within a run share agents.** A mixed-effects model would be better for the full study.
5. **Not pre-registered.** Follow-up should be.
...
---
*Generated by `full_analysis.py` - 2026-02-18 13:52*