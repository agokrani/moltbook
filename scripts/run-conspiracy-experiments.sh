#!/usr/bin/env bash
# ============================================
# Conspiracy Experiment Runner
# ============================================
#
# Runs 6 conspiracy experiments in 3 batches of 2 parallel slots:
#   Batch 1: c1-run01 (baseline) + c2-run01 (nudge×veracity)
#   Batch 2: c3-run01 (correction) + c5a-run01 (80% factual)
#   Batch 3: c5b-run01 (50/50) + c5c-run01 (80% conspiracy)
#
# Usage:
#   ./scripts/run-conspiracy-experiments.sh [--duration <secs>] [--dry-run]
#
...
LOG_DIR="$PROJECT_DIR/experiments/conspiracy/parallel-logs"

# Batches: each entry is "ENV_PRESET EXPERIMENT_NAME"
declare -a BATCH1=("c1 c1-run01" "c2 c2-run01")
declare -a BATCH2=("c3 c3-run01" "c5a c5a-run01")
declare -a BATCH3=("c5b c5b-run01" "c5c c5c-run01")
...
while [[ $# -gt 0 ]]; do
  case $1 in
    ...
    *)
      ...
      echo "Usage: $0 [--duration secs] [--dry-run]"
      exit 1
      ;;
  esac
done
...
# ============================================
log "============================================"
log "Conspiracy Experiment Runner"
log "============================================"
...
log ""
log "Schedule:"
log "  Batch 1: c1-run01 (baseline, 50 posts) + c2-run01 (nudge×veracity, 50 posts)"
log "  Batch 2: c3-run01 (correction, 25 conspiracy) + c5a-run01 (80% factual, 25 posts)"
log "  Batch 3: c5b-run01 (50/50, 25 posts) + c5c-run01 (80% conspiracy, 25 posts)"
log ""
...
# ============================================
for ENV in c1 c2 c3 c5a c5b c5c; do
  if [ ! -f "$PROJECT_DIR/.env.${ENV}" ]; then
    log "[ERROR] Missing .env.${ENV}"
    exit 1
  fi
done

for WP in world-posts-all.jsonl world-posts-conspiracy-only.jsonl world-posts-80fact.jsonl world-posts-50-50.jsonl world-posts-80cons.jsonl; do
  if [ ! -f "$PROJECT_DIR/experiments/conspiracy/$WP" ]; then
    log "[ERROR] Missing experiments/conspiracy/$WP — run generate-world-posts.py first"
    exit 1
  fi
done
...
slot_worker() {
  ...
  cp "$PROJECT_DIR/.env.${ENV_PRESET}" "$SLOT_ENV"
  if [ $? -ne 0 ]; then
    slog "[ERROR] Failed to copy .env.${ENV_PRESET}"
...
run_batch() {
  ...
  for i in "${!ENTRIES[@]}"; do
    ...
    local ENV_PRESET=$(echo "$ENTRY" | cut -d' ' -f1)
    local RUN_NAME=$(echo "$ENTRY" | cut -d' ' -f2)

    log "[SLOT $i] Starting $RUN_NAME (env: .env.${ENV_PRESET})"
    slot_worker "$i" "$ENV_PRESET" "$RUN_NAME" &
    PIDS+=($!)
    NAMES+=("$RUN_NAME")
  done
...
log "============================================"
log "Starting Conspiracy Experiments"
log "============================================"
log ""
...
for RUN_NAME in c1-run01 c2-run01 c3-run01 c5a-run01 c5b-run01 c5c-run01; do
  STATUS="MISSING"
  if [ -f "$LOG_DIR/${RUN_NAME}.status" ]; then
    STATUS=$(cat "$LOG_DIR/${RUN_NAME}.status")
  fi
  if [ -d "$PROJECT_DIR/exports/$RUN_NAME" ]; then
    ...
    POSTS=$(jq -r '.stats.posts // "?"' "$PROJECT_DIR/exports/$RUN_NAME/metadata.json" 2>/dev/null || echo "?")
    log "  $RUN_NAME  [$STATUS]  ${POSTS} posts  ($SIZE)"
  else
