#!/usr/bin/env bash
# ============================================
# Threshold Dose-Response Experiment Runner
# ============================================
#
# Runs 6 threshold experiments in 3 batches of 2 parallel slots:
#   Batch 1: th-f0-run01 (0F+25C) + th-f1-run01 (1F+24C)
#   Batch 2: th-f2-run01 (2F+23C) + th-f3-run01 (3F+22C)
#   Batch 3: th-f4-run01 (4F+21C) + th-f5-run01 (5F+20C)
#
# All runs: Mode C (no nudges), GPT-5, 1 hour, 25 posts at 120s intervals.
#
# Usage:
#   ./scripts/run-threshold-experiments.sh [--duration <secs>] [--dry-run]
#
...
BASE_ENV="$PROJECT_DIR/.env.threshold-base"
...
# ============================================
declare -a BATCH1=("0 th-f0-run${RUN_NUMBER}" "1 th-f1-run${RUN_NUMBER}")
declare -a BATCH2=("2 th-f2-run${RUN_NUMBER}" "3 th-f3-run${RUN_NUMBER}")
declare -a BATCH3=("4 th-f4-run${RUN_NUMBER}" "5 th-f5-run${RUN_NUMBER}")
...
# ============================================
log "============================================"
log "Threshold Dose-Response Experiment Runner"
log "============================================"
...
log ""
log "Schedule:"
log "  Batch 1: th-f0-run${RUN_NUMBER} (0F+25C) + th-f1-run${RUN_NUMBER} (1F+24C)"
log "  Batch 2: th-f2-run${RUN_NUMBER} (2F+23C) + th-f3-run${RUN_NUMBER} (3F+22C)"
log "  Batch 3: th-f4-run${RUN_NUMBER} (4F+21C) + th-f5-run${RUN_NUMBER} (5F+20C)"
log ""
...
for N in 0 1 2 3 4 5; do
  WP="$PROJECT_DIR/experiments/conspiracy/world-posts-f${N}.jsonl"
  if [ ! -f "$WP" ]; then
    log "[ERROR] Missing $WP — run generate-threshold-posts.py first"
    exit 1
  fi
done
...
slot_worker() {
  ...
  if [ $? -ne 0 ]; then
    slog "[ERROR] Failed to copy .env.threshold-base"
  ...
  echo "WORLD_POSTS_FILE=/app/experiments/conspiracy/world-posts-f${N_FACTUAL}.jsonl" >> "$SLOT_ENV"
...
log "============================================"
log "Starting Threshold Dose-Response Experiments"
log "============================================"
log ""
...
log ""
log "============================================"
log "ALL THRESHOLD EXPERIMENTS COMPLETE"
log "============================================"
...
for N in 0 1 2 3 4 5; do
  RUN_NAME="th-f${N}-run${RUN_NUMBER}"
  STATUS="MISSING"
  if [ -f "$LOG_DIR/${RUN_NAME}.status" ]; then
    STATUS=$(cat "$LOG_DIR/${RUN_NAME}.status")
  fi
...
log ""
log "Next step: python3 experiments/conspiracy/analyze_threshold.py"
