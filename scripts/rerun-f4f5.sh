#!/usr/bin/env bash
# Re-run only fc-f4-run01 and fc-f5-run01
...
rm -rf "$PROJECT_DIR/exports/fc-f4-run01" "$PROJECT_DIR/exports/fc-f5-run01"
rm -f "$PROJECT_DIR/experiments/factcheck/parallel-logs/fc-f4-run01.status"
rm -f "$PROJECT_DIR/experiments/factcheck/parallel-logs/fc-f5-run01.status"
...
log "============================================"
log "Re-running fc-f4-run01 + fc-f5-run01"
log "============================================"

PIDS=() NAMES=()
log "[SLOT 0] Starting fc-f4-run01 ((4F + 21C))"
slot_worker 0 4 "fc-f4-run01" &
PIDS+=($!) NAMES+=("fc-f4-run01")

log "[SLOT 1] Starting fc-f5-run01 ((5F + 20C))"
slot_worker 1 5 "fc-f5-run01" &
PIDS+=($!) NAMES+=("fc-f5-run01")
