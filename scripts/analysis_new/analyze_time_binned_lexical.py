#!/usr/bin/env python3
"""Compute strict 60-minute lexical metrics in fixed time bins."""
...
from time_binned_lexical_metrics import (  # noqa: E402
    fixed_time_bins,
    lexical_metrics,
    prepare_posts,
...
DEFAULT_OUT_DIR = Path("findings/entropy-collapse-multiscale-new")
PLOT_COLORS = {
    "n10": "#1f77b4",
    "n20": "#ff7f0e",
    "n30": "#2ca02c",
...
def main() -> None:
        ...
        for condition in CONDITION_ORDER:
            ...
            for _, _, run_name in run_keys:
                ...
                for bin_idx, start, end, members in bins:
                    ...
                    d1 = subsampled_distinct_n(
                        members,
                        1,
                        target_size,
                        n_samples=args.n_samples,
                        seed=args.seed + bin_idx + 11,
                    )
                    d2 = subsampled_distinct_n(
                        members,
                        2,
                        target_size,
                        n_samples=args.n_samples,
                        seed=args.seed + bin_idx + 23,
                    )
                    d3 = subsampled_distinct_n(
                        members,
                        3,
                        target_size,
                        n_samples=args.n_samples,
                        seed=args.seed + bin_idx + 37,
                    )
                    subsampled_row = {
                        "scale": scale,
                        "condition": condition,
                        ...
                        "n_posts": len(members),
                        "subsample_target_posts": target_size,
                        "distinct_1_mean": d1["mean"],
                        "distinct_1_ci_lo": d1["ci_lo"],
                        "distinct_1_ci_hi": d1["ci_hi"],
                        "distinct_2_mean": d2["mean"],
                        "distinct_2_ci_lo": d2["ci_lo"],
                        "distinct_2_ci_hi": d2["ci_hi"],
                        "distinct_3_mean": d3["mean"],
                        "distinct_3_ci_lo": d3["ci_lo"],
                        "distinct_3_ci_hi": d3["ci_hi"],
                    }
                    ...
                    run_json["bins"].append(
                        {
                            ...
                            "subsampled": {
                                ...
                                "distinct_1": {k: d1[k] for k in ("mean", "ci_lo", "ci_hi")},
                                "distinct_2": {k: d2[k] for k in ("mean", "ci_lo", "ci_hi")},
                                "distinct_3": {k: d3[k] for k in ("mean", "ci_lo", "ci_hi")},
                            },
                        }
                    )
    ...
    plot_distinct(
        ...
        title="Raw distinct-n by fixed 15-minute bin",
        metric_keys=("distinct_1_mean", "distinct_2_mean", "distinct_3_mean"),
    )
    plot_distinct(
        subsampled_summary_rows,
        out_path=plots_dir / "distinct_n_subsampled_by_scale.png",
        title="Subsampled distinct-n by fixed 15-minute bin",
        metric_keys=("distinct_1_mean", "distinct_2_mean", "distinct_3_mean"),
    )
