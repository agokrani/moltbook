...
"""Upload cleaned MoltBook Entropy Collapse GLM-5 experiment results to HuggingFace."""
...
if not HF_TOKEN:
    try:
        ...
        _api = HfApi()
        _api.whoami()  # will work if logged in via huggingface-cli
        HF_TOKEN = _api.token
    except Exception:
...
REPO_ID = "Ayushnangia/moltbook-entropy-collapse-glm-5"
...
# GLM-5 result dirs — naming is inconsistent so we map explicitly
GLM_DIR_MAP = {
    "mag0": "ec-mag0-n10-run01-glm5-20260317",
    "mag1": "ec-mag1-n10-run01",
    "mag5": "ec-mag5-n10-run01",
    "mag25": "ec-mag25-n10-run01",
    "dom-agi": "ec-dom-agi-n10-run01",
    "dom-tech": "ec-dom-tech-n10-run01",
}

# Canonical output names (consistent naming for HF)
CANONICAL_NAME = {
    "mag0": "ec-mag0-n10-run01",
    "mag1": "ec-mag1-n10-run01",
    "mag5": "ec-mag5-n10-run01",
    "mag25": "ec-mag25-n10-run01",
    "dom-agi": "ec-dom-agi-n10-run01",
...
EXPERIMENT_DIRS = []
for cond, dirname in GLM_DIR_MAP.items():
    d = SCRATCH_DIR / dirname
    if d.exists() and (d / "metadata.json").exists():
        meta = json.loads((d / "metadata.json").read_text())
        if "glm" in meta.get("model", "").lower():
            EXPERIMENT_DIRS.append((cond, d))
        else:
            print(f"[WARN] {dirname} model is '{meta.get('model')}', not GLM — skipping")
    else:
...
tags:
  ...
  - glm-5
  - z-ai
pretty_name: "MoltBook Entropy Collapse Experiments — GLM-5"
size_categories:
  - 1K<n<10K
---

# MoltBook Entropy Collapse Experiments — GLM-5

Multi-agent social simulation data from the **Entropy Collapse** experiment series run on [MoltBook](https://github.com/agokrani/moltbook), a Reddit-like social network for AI agents. This dataset uses **Z-AI GLM-5** as the underlying LLM.
...
```bibtex
@dataset{{moltbook_entropy_collapse_glm5_2026,
  title={{MoltBook Entropy Collapse Experiments — GLM-5}},
  author={{Nangia, Ayush}},
  year={{2026}},
  url={{https://huggingface.co/datasets/{REPO_ID}}},
  note={{Multi-agent social simulation on MoltBook platform using GLM-5}}
}}
```
...
def main():
    ...
    print(f"\nFound {len(EXPERIMENT_DIRS)} GLM-5 experiment directories:")
    ...
    staging = Path(tempfile.mkdtemp(prefix="hf_upload_glm5_"))
    ...
    api.upload_folder(
        ...
        commit_message="GLM-5 entropy collapse experiments: 6 conditions, 10 agents, 1h each",
    )
