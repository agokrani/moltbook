...
"""Upload cleaned MoltBook Entropy Collapse Gemini 3.1 Flash Lite experiment results to HuggingFace."""
...
REPO_ID = "Ayushnangia/moltbook-entropy-collapse-gemini-flash-lite-n20"
...
# Gemini result dirs
GEMINI_DIR_MAP = {
    "mag0": "ec-mag0-n20-run01-gemini-3.1-flash-lite-preview-20260318",
    "mag1": "ec-mag1-n20-run01-gemini-3.1-flash-lite-preview-20260318",
    "mag5": "ec-mag5-n20-run01-gemini-3.1-flash-lite-preview-20260318",
    "mag25": "ec-mag25-n20-run01-gemini-3.1-flash-lite-preview-20260318",
    "dom-agi": "ec-dom-agi-n20-run01-gemini-3.1-flash-lite-preview-20260318",
    "dom-tech": "ec-dom-tech-n20-run01-gemini-3.1-flash-lite-preview-20260318",
}

# Canonical output names (consistent for HF)
CANONICAL_NAME = {
    "mag0": "ec-mag0-n20-run01",
    "mag1": "ec-mag1-n20-run01",
    "mag5": "ec-mag5-n20-run01",
    "mag25": "ec-mag25-n20-run01",
    "dom-agi": "ec-dom-agi-n20-run01",
    "dom-tech": "ec-dom-tech-n20-run01",
}

# Resolve and validate experiment dirs
EXPERIMENT_DIRS = []
for cond, dirname in GEMINI_DIR_MAP.items():
    d = SCRATCH_DIR / dirname
    if d.exists() and (d / "metadata.json").exists():
        meta = json.loads((d / "metadata.json").read_text())
        if "gemini" in meta.get("model", "").lower():
            EXPERIMENT_DIRS.append((cond, d))
        else:
            print(f"[WARN] {dirname} model is '{meta.get('model')}', not Gemini — skipping")
    else:
        print(f"[WARN] {dirname} not found or missing metadata — skipping")

EXPERIMENT_DIRS.sort(key=lambda x: x[0])
...
tags:
  ...
  - gemini
  - google
pretty_name: "MoltBook Entropy Collapse Experiments — Gemini 3.1 Flash Lite (n20)"
size_categories:
  - 1K<n<10K
---

# MoltBook Entropy Collapse Experiments — Gemini 3.1 Flash Lite (n20)

Multi-agent social simulation data from the **Entropy Collapse** experiment series run on [MoltBook](https://github.com/agokrani/moltbook), a Reddit-like social network for AI agents. This dataset uses **Google Gemini 3.1 Flash Lite** as the underlying LLM with **20 agents (n20)**.
...
- **Agents per run**: 20 (alpha through upsilon)
...
@dataset{{moltbook_entropy_collapse_gemini_2026,
  ...
  note={{Multi-agent social simulation on MoltBook platform using Gemini 3.1 Flash Lite (n20 agents)}}
}}
```
...
def main():
    ...
    print(f"\nFound {len(EXPERIMENT_DIRS)} Gemini experiment directories:")
    for cond, d in EXPERIMENT_DIRS:
        print(f"  [{cond}] {d.name}")

    if len(EXPERIMENT_DIRS) != 6:
        print(f"[ERROR] Expected 6 conditions, found {len(EXPERIMENT_DIRS)}")
        sys.exit(1)
    ...
    staging = Path(tempfile.mkdtemp(prefix="hf_upload_gemini_"))
    print(f"\nStaging cleaned data in {staging}")

    stats_rows = []

    for cond, exp_dir in EXPERIMENT_DIRS:
    ...
    api.upload_folder(
        ...
        commit_message="Gemini 3.1 Flash Lite entropy collapse experiments: 6 conditions, 10 agents, 1h each",
    )
