...
"""Upload best clean MoltBook Entropy Collapse Gemini 3.1 Flash Lite runs (n10+n20+n30) to HuggingFace.

Picks the latest directory WITHOUT failure suffixes (-garbage, -collapsed, -9of10, etc.)
for each condition+scale combination. Validates metadata contains 'gemini'.
"""
...
# Failure suffixes that indicate model-behaviour failures
FAILURE_SUFFIXES = ("-garbage", "-collapsed", "-9of10", "-27of30", "-28of30")
# Infrastructure failure suffixes — also not clean
INFRA_SUFFIXES = ("-partial", "-cancelled", "-portcrash")
ALL_BAD_SUFFIXES = FAILURE_SUFFIXES + INFRA_SUFFIXES

# Best clean directories per (condition, scale).
# These are the latest directories WITHOUT any failure/infra suffix.
# n10: mag0/mag1/dom-tech ran on 0317, dom-agi/mag25/mag5 re-ran clean on 0318
# n20/n30: all clean runs are on 0318
EXPERIMENT_MAP = {
    # n10
    ...
    ("mag5", 10): "ec-mag5-n10-run01-gemini-3.1-flash-lite-preview-20260318",
    ("mag25", 10): "ec-mag25-n10-run01-gemini-3.1-flash-lite-preview-20260318",
    ("dom-agi", 10): "ec-dom-agi-n10-run01-gemini-3.1-flash-lite-preview-20260318",
...
for (cond, n), dirname in sorted(EXPERIMENT_MAP.items()):
    ...
    if not d.exists():
        print(f"[WARN] {dirname} does not exist — skipping")
        continue
    meta_path = d / "metadata.json"
    if not meta_path.exists():
        print(f"[WARN] {dirname} has no metadata.json — skipping")
        continue
    meta = json.loads(meta_path.read_text())
    if "gemini" not in meta.get("model", "").lower():
        print(f"[WARN] {dirname} model is '{meta.get('model')}', not Gemini — skipping")
        continue
    # Verify no bad suffix leaked in
    for sfx in ALL_BAD_SUFFIXES:
        if dirname.endswith(sfx):
            print(f"[WARN] {dirname} has failure suffix '{sfx}' — skipping")
            break
    else:
...
|-------|-----------|----------------|---------------------|
| n10 | {sum(r['posts'] for r in n10_rows):,} | {sum(r['comments'] for r in n10_rows):,} | {sum(r['posts'] for r in n10_rows) / max(10 * len(n10_rows), 1):.1f} |
| n20 | {sum(r['posts'] for r in n20_rows):,} | {sum(r['comments'] for r in n20_rows):,} | {sum(r['posts'] for r in n20_rows) / max(20 * len(n20_rows), 1):.1f} |
| n30 | {sum(r['posts'] for r in n30_rows):,} | {sum(r['comments'] for r in n30_rows):,} | {sum(r['posts'] for r in n30_rows) / max(30 * len(n30_rows), 1):.1f} |
...
def main():
    ...
    print(f"\nFound {len(EXPERIMENTS)} clean experiment directories:")
    ...
    staging = Path(tempfile.mkdtemp(prefix="hf_upload_gemini_clean_"))
    ...
    api.upload_folder(
        ...
        commit_message="Gemini 3.1 Flash Lite: best clean runs, 6 conditions x 3 scales (n10/n20/n30), 18 runs total",
    )
