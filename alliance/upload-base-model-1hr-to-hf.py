...
"""Upload 1-hour base model experiment results (qwen-base, 6 conditions) to HuggingFace."""
...
REPO_ID = "Ayushnangia/moltbook-ec-1h-base-model-experiments"
...
SOURCE_DIR = Path("/scratch/anangia/moltbook/results/base-model-new-1hr")
...
def build_readme(stats):
    ...
    total_posts = sum(s["posts"] for s in stats)
...
def main():
    ...
    staging = Path(tempfile.mkdtemp(prefix="hf_upload_base_model_1hr_"))
    stats = []

    print(f"\n--- Qwen 3.5 35B A3B Base (1h runs) ---")
    ...
    api.upload_folder(
        ...
        commit_message="Base model experiments: Qwen 3.5 35B A3B Base, 6 conditions, 10 agents, 1 hour each",
    )
